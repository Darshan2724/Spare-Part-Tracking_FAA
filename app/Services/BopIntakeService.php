    ...
    public function getAggregatedBopParts(array $filters = [], int $page = 1, int $perPage = 50): array
    {
        ...
        foreach ($grouped as $partNo => $items) {
            ...
            $aggregatedRows->push([
                ...
                'completion_pct' => $totalRequired > 0 ? min(100, max(0, (int) round(($assemblyCompleted / $totalRequired) * 100))) : 0,
                'received_pct' => $totalRequired > 0 ? min(100, max(0, (int) round(($totalReceived / $totalRequired) * 100))) : 0,
                'status' => $status,
