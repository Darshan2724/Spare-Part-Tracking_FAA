...
# ============================================================================
# SECTION 6: POSTGRESQL DIAGNOSTICS (READ-ONLY SQL VIA docker exec)
# ============================================================================
...
RunSQL "Existing Indexes and Usage" "SELECT schemaname || '.' || relname AS table_name, indexrelname AS index_name, idx_scan AS times_used, pg_size_pretty(pg_relation_size(indexrelid)) AS index_size FROM pg_stat_user_indexes ORDER BY idx_scan DESC LIMIT 30;"
...
# ============================================================================
# SECTION 7: API ENDPOINT TIMING (localhost - zero LAN overhead)
# ============================================================================
LogSection "SECTION 7: API ENDPOINT TIMING (localhost)"

Log "Timing API endpoints from the server itself (127.0.0.1:8080)"
Log "This isolates pure server processing time from any LAN latency."
Log ""
...
function TimeEndpoint($method, $path, $label, $iterations) {
    if (-not $iterations) { $iterations = 3 }
...
try {
    ...
    Log ("Could not get project list: " + $_.Exception.Message)
}

# Time each endpoint
TimeEndpoint "GET" "health" "Health Check (baseline)" 3

TimeEndpoint "GET" "dashboard/summary" "Dashboard Summary - ALL types (HEAVIEST)" 3

TimeEndpoint "GET" "dashboard/summary?part_type=MFG" "Dashboard Summary - MFG only" 2

TimeEndpoint "GET" "dashboard/summary?part_type=BOP" "Dashboard Summary - BOP only" 2

TimeEndpoint "GET" "dashboard/summary?part_type=STD" "Dashboard Summary - STD only" 2

TimeEndpoint "GET" "dashboard/project-hierarchy" "Project Hierarchy (project list)" 2

if ($testProjectId) {
    TimeEndpoint "GET" ("dashboard/project-hierarchy?project_id=" + $testProjectId) "Project Hierarchy (full drill-down)" 2
    TimeEndpoint "GET" ("dashboard/project-hierarchy?project_id=" + $testProjectId + "&part_type=MFG") "Project Hierarchy (MFG drill-down)" 2
}

TimeEndpoint "GET" "dashboard/kpi-drilldown?kpi=total_parts" "KPI Drilldown" 2

TimeEndpoint "GET" "store/hierarchy" "Store Department Hierarchy" 2

TimeEndpoint "GET" "qc/hierarchy" "QC Department Hierarchy" 2

TimeEndpoint "GET" "rework/hierarchy" "Rework Department Hierarchy" 2

TimeEndpoint "GET" "paint/hierarchy" "Paint Department Hierarchy" 2

TimeEndpoint "GET" "assembly/hierarchy" "Assembly Department Hierarchy" 2

TimeEndpoint "GET" "ecn/dashboard/summary" "ECN Dashboard Summary" 2
...
if ($testProjectId) {
    TimeEndpoint "GET" ("store/hierarchy?project_id=" + $testProjectId) "Store Hierarchy (project drill-down)" 2
    TimeEndpoint "GET" ("qc/hierarchy?project_id=" + $testProjectId) "QC Hierarchy (project drill-down)" 2
}

# ============================================================================
# SECTION 8: SUMMARY
# ============================================================================
LogSection "SECTION 8: DIAGNOSTIC COMPLETE"
...
Log ""
Log ("Diagnostic completed in " + $totalSec + " seconds")
Log ("All results saved to: " + $logFile)
Log ""
Log "NEXT STEPS:"
Log "  1. Share the file back (USB or git push)"
Log "  2. To git push:"
Log ("     git add " + $logFile)
...
Write-Host "  App is still running - nothing changed." -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
