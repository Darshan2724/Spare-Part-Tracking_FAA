<template>
  <div class="p-4" style="background-color: #f8fafc; min-height: 100vh;">
      ...
      <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-bottom py-3">
          <div class="d-flex justify-content-between align-items-center">
            <div>
              ...
              <p class="text-muted mb-0">Track rework cycles by Project hierarchy, perform corrections, and submit parts back to QC queue.</p>
            </div>
            <span class="badge bg-danger px-3 py-2 fs-6">Rework Center</span>
          </div>
        </div>
        ...
        <div class="card-body">
              ...
              <table class="table table-hover align-middle border-top mb-0">
                <thead class="table-dark">
                  <tr>
                    ...
                    <th>Project</th>
                    <th>Side</th>
                    <th>Rework Load (Pending / In Progress)</th>
                    <th>Completed Cycles</th>
                    <th style="width: 220px;">Rework Action</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="part in searchResults" :key="part.id">
                    ...
                    <td>
                      <span v-if="part.metrics.rework_pending > 0" class="badge bg-warning text-dark me-1">
                        <i class="fas fa-clock me-1"></i>{{ part.metrics.rework_pending }} pcs Pending Start
                      </span>
                      <span v-if="part.metrics.rework_in_progress > 0" class="badge bg-primary">
                        <i class="fas fa-tools me-1"></i>{{ part.metrics.rework_in_progress }} pcs In Progress
                      </span>
                      <span v-if="part.metrics.rework_pending === 0 && part.metrics.rework_in_progress === 0" class="text-muted small">
                        No active rework items
                      </span>
                    </td>
                    <td>
                      <span class="badge bg-success" v-if="part.metrics.rework_completed > 0">
                        {{ part.metrics.rework_completed }} pcs Returned to QC
                      </span>
                      <span v-else class="text-muted small">—</span>
                    </td>
                    <td>
                      <div v-if="authStore.userRole === 'ADMIN'" class="d-flex flex-column gap-1">
                        <button v-if="part.metrics.rework_pending > 0" class="btn btn-sm btn-primary fw-bold text-nowrap" @click="startReworkFromPart(part)">
                          <i class="fas fa-play me-1"></i> Start Rework
                        </button>
                        <button v-if="part.metrics.rework_in_progress > 0" class="btn btn-sm btn-success fw-bold text-nowrap" @click="openCompleteModalFromPart(part)">
                          <i class="fas fa-check-circle me-1"></i> Complete & Return to QC
                        </button>
                      </div>
                      <div v-else class="text-center text-muted extra-small">
                        <span v-if="part.metrics.rework_in_progress > 0" class="badge bg-primary-subtle text-primary border">In Progress</span>
                        <span v-else-if="part.metrics.rework_pending > 0" class="badge bg-warning-subtle text-dark border">Pending</span>
                  ...
                  <tr v-if="!searchResults.length">
                    <td colspan="6" class="text-center py-5 text-muted">
                      ...
                      <p class="mb-0 fs-6">No matching rework parts found for "{{ searchQuery }}".</p>
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div v-else-if="!projectId">
            <h6 class="fw-bold mb-3 text-secondary text-uppercase ls-1"><i class="fas fa-project-diagram me-2 text-primary"></i>Active Projects Rework Load ({{ projects.length }})</h6>
            <div class="row g-2">
                  ...
                  <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                      <h6 class="fw-bold mb-0 text-dark text-truncate me-2" style="max-width: 70%;" :title="proj.name">
                        <i class="fas fa-folder text-danger me-1.5"></i>{{ proj.name || proj.project_code || 'Project' }}
                      </h6>
                    ...
                    <div class="d-flex justify-content-between text-muted extra-small border-top pt-1.5">
                      ...
                      <span>QC Cleared: <strong class="text-success">{{ proj.qc_approved }}</strong></span>
                    </div>
                  </div>
                  <div class="card-footer bg-light border-0 text-end py-1.5 px-3">
                    <span class="extra-small text-primary fw-bold">Open Rework Tree <i class="fas fa-arrow-right ms-1"></i></span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-else>
            ...
            <div v-if="!selectedJig">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <h6 class="fw-bold mb-0 text-dark">
                  <i class="fas fa-cubes me-1.5 text-danger"></i>JIG Assemblies for {{ activeProject?.name }} ({{ jigs.length }} JIGs)
                </h6>
              </div>
                    ...
                    <div class="card-body p-3">
                      <div class="d-flex justify-content-between align-items-start mb-1">
                        <h6 class="fw-bold mb-0 text-dark text-truncate me-2">
                          <i class="fas fa-layer-group text-danger me-1.5"></i>{{ jig.jig_name }}
                        </h6>
                        <span class="badge bg-secondary" style="font-size: 0.72rem;">
                          {{ jig.units?.length || 0 }} Units
                        </span>
                      </div>
                      ...
                      <div class="d-flex justify-content-between text-muted extra-small border-top pt-1.5">
                        <span>Pending: <strong class="text-danger">{{ jig.metrics?.rework_pending || 0 }}</strong></span>
                        <span>Active: <strong class="text-primary">{{ jig.metrics?.rework_in_progress || 0 }}</strong></span>
                        <span>Done: <strong class="text-success">{{ jig.metrics?.rework_completed || 0 }}</strong></span>
                      </div>
                    </div>
                    <div class="card-footer bg-light border-0 text-end py-1.5 px-3">
            ...
            <div v-else-if="selectedJig && !selectedUnit">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <h6 class="fw-bold mb-0 text-dark">
                  <i class="fas fa-th-large me-1.5 text-danger"></i>Units in JIG {{ selectedJig.jig_name }} ({{ selectedJig.units?.length || 0 }} Units)
                </h6>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center mb-2 gap-2">
                  <div>
                    <h6 class="fw-bold mb-0 text-dark d-flex align-items-center flex-wrap gap-2">
                      <span><i class="fas fa-list me-1.5 text-danger"></i>JIG {{ selectedJig.jig_name }} — {{ selectedUnit.unit_no }}</span>
                ...
                <div class="card border shadow-xs">
                  <div class="card-header py-2 px-3 border-bottom d-flex justify-content-between align-items-center"
                    ...
                    <div class="d-flex align-items-center gap-2">
                      ...
                      <h6 class="fw-bold mb-0 text-dark">{{ selectedUnit.unit_no }} — {{ selectedUnitSide === 'COMMON' ? 'Common' : (selectedUnitSide === 'LH' ? 'Left Hand' : 'Right Hand') }} Rework Bay</h6>
                    </div>
                      ...
                      <table class="table table-hover align-middle border-top mb-0 small">
                        <thead class="table-dark sticky-top" style="z-index: 1;">
                          <tr>
                            ...
                            <th>Rework Status</th>
                            <th>Remark / Notes</th>
                            <th style="width: 150px;">{{ authStore.userRole === 'ADMIN' ? 'Rework Action' : 'Status' }}</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr v-for="part in activeUnitSideParts" :key="selectedUnitSide + '_' + part.id">
                            ...
                            <td>
                              <div v-if="part.metrics.rework_pending > 0" class="badge bg-warning text-dark d-block mb-1">
                                <i class="fas fa-clock me-1"></i>{{ part.metrics.rework_pending }} Pending
                              </div>
                              <div v-if="part.metrics.rework_in_progress > 0" class="badge bg-primary d-block mb-1">
                                <i class="fas fa-tools me-1"></i>{{ part.metrics.rework_in_progress }} In Progress
                              </div>
                              <div v-if="part.metrics.rework_completed > 0" class="badge bg-success d-block">
                                <i class="fas fa-check-circle me-1"></i>{{ part.metrics.rework_completed }} Done
                              </div>
                            </td>
                            <td>
                              <div v-if="part.rework_remark || getLatestReworkRemark(part)" class="d-flex align-items-center gap-1">
                                <span class="text-truncate extra-small fw-semibold text-dark" style="max-width: 160px;" :title="part.rework_remark || getLatestReworkRemark(part)">
                                  <i class="fas fa-comment-dots text-warning me-1"></i>{{ part.rework_remark || getLatestReworkRemark(part) }}
                                </span>
                                <button class="btn btn-xs btn-outline-secondary py-0 px-1" @click="viewFullRemark(part)" title="View full remark">
                                  <i class="fas fa-eye"></i>
                                </button>
                              </div>
                              <span v-else class="text-muted extra-small fst-italic">No remarks</span>
                            </td>
                            <td>
                              <div v-if="authStore.userRole === 'ADMIN'" class="d-flex flex-column gap-1">
                                <button v-if="part.metrics.rework_pending > 0" class="btn btn-xs btn-primary fw-bold text-nowrap w-100" @click="startReworkFromPart(part)">
                                  <i class="fas fa-play me-1"></i> Start
                                </button>
                                <button v-if="part.metrics.rework_in_progress > 0" class="btn btn-xs btn-success fw-bold text-nowrap w-100" @click="openCompleteModalFromPart(part)">
                                  <i class="fas fa-check-circle me-1"></i> Complete
                                </button>
                                <span v-if="part.metrics.rework_pending === 0 && part.metrics.rework_in_progress === 0" class="text-muted extra-small text-center">
                                  <i class="fas fa-check text-success me-1"></i>No Queue
                                </span>
                              </div>
                          ...
                          <tr v-if="!(selectedUnitSide === 'LH' ? selectedUnitLhParts : selectedUnitRhParts).length">
                            <td colspan="5" class="text-center py-4 text-muted">No {{ selectedUnitSide }} rework parts in this unit.</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
    ...
    <div class="modal fade" id="completeModal" tabindex="-1">
      <div class="modal-dialog">
        <div class="modal-content shadow-lg border-0">
          ...
          <div class="modal-body" v-if="activeReworkRecord">
            <p>You are marking rework as completed for <strong>{{ activeReworkRecord.standard_part_no }}</strong> ({{ activeReworkRecord.quantity }} units, {{ activeReworkRecord.side }}).</p>
            <div class="mb-3">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-success fw-bold" @click="submitCompletion" :disabled="submitting">
    ...
    <!-- View Full Remark Modal -->
    <div v-if="showRemarkModal" class="modal fade show d-block" tabindex="-1" style="background: rgba(0,0,0,0.5); z-index: 1060;">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow-lg border-0">
          <div class="modal-header bg-dark text-white py-2 px-3">
            <h6 class="modal-title fw-bold mb-0">
              <i class="fas fa-sticky-note me-2 text-warning"></i>Rework Remark Details
            </h6>
            <button type="button" class="btn-close btn-close-white" @click="showRemarkModal = false"></button>
          </div>
          <div class="modal-body p-3">
            <div class="mb-2 d-flex gap-2">
              <span class="badge bg-primary">{{ selectedRemarkPart?.standard_part_no }}</span>
              <span class="badge bg-secondary">{{ selectedUnitSide }} Side</span>
              <span class="badge bg-info text-dark" v-if="selectedRemarkPart?.jig_name">{{ selectedRemarkPart?.jig_name }}</span>
            </div>
            <div class="p-3 bg-light rounded border text-dark font-monospace small" style="white-space: pre-wrap; line-height: 1.5;">
              {{ selectedRemarkText }}
            </div>
          </div>
          <div class="modal-footer py-2">
            <button type="button" class="btn btn-sm btn-secondary" @click="showRemarkModal = false">Close</button>
          </div>
        </div>
      </div>
    </div>
...
const selectedRemarkPart = ref(null);
const selectedRemarkText = ref('');

const getLatestReworkRemark = (part) => {
  const records = part.rework_records || [];
  if (!records.length) return null;
  const latest = records[records.length - 1];
  return latest.completion_notes || latest.rework_description || null;
};

const viewFullRemark = (part) => {
  selectedRemarkPart.value = part;
  selectedRemarkText.value = part.rework_remark || getLatestReworkRemark(part) || 'No remarks recorded.';
...
const activeReworkRecord = ref(null);
...
const onSearchInput = () => {
  ...
  searchDebounce = setTimeout(async () => {
    ...
    try {
      const response = await axios.get(`/api/v1/rework/hierarchy?search=${encodeURIComponent(searchQuery.value)}&project_id=${projectId.value || ''}&side=${selectedSide.value || ''}`);
...
const startReworkFromPart = async (part) => {
  const record = (part.rework_records || []).find(r => r.status === 'pending');
  if (!record) {
    error.value = 'No pending rework record found for this part.';
    return;
  }
...
const openCompleteModalFromPart = (part) => {
  const record = (part.rework_records || []).find(r => r.status === 'in_progress');
  if (!record) {
    error.value = 'No in-progress rework record found for this part.';
    return;
  }
  activeReworkRecord.value = {
    ...record,
...
const submitCompletion = async () => {
  submitting.value = true;
  try {
    const res = await axios.post(`/api/v1/rework/items/${activeReworkRecord.value.id}/complete`, {
