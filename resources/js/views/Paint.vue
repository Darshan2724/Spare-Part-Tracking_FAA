      ...
      <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-bottom py-3">
          <div class="d-flex justify-content-between align-items-center">
            <div>
              <h3 class="fw-bold mb-1"><i class="fas fa-paint-roller me-2 text-purple"></i>Paint Shop Operations</h3>
              <p class="text-muted mb-0">Surface coating and painting for QC-approved spare parts by Project hierarchy.</p>
            </div>
            <span class="badge bg-purple text-white px-3 py-2 fs-6">Paint Shop</span>
          </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="col-md-4">
              ...
              <select v-model="projectId" class="form-select form-select-lg shadow-xs" @change="onProjectChange">
                ...
                <option v-for="project in projects" :key="project.id" :value="project.id">
                  {{ project.name || project.project_code }} ({{ project.completion_pct }}% Paint Done)
                </option>
              </select>
            </div>
            <div class="col-md-3">
              ...
              <table class="table table-hover align-middle border-top mb-0">
                <thead class="table-dark">
                  <tr>
                    ...
                    <th>Project</th>
                    <th>Side</th>
                    <th>QC Approved / Ready for Paint</th>
                    <th>Paint Completed</th>
                    <th style="width: 200px;">Paint Action</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="part in searchResults" :key="part.id">
                    ...
                    <td>
                      <span v-if="part.metrics.paint_ready > 0" class="badge bg-warning text-dark">
                        <i class="fas fa-paint-roller me-1"></i>{{ part.metrics.paint_ready }} pcs Ready for Paint
                      </span>
                      <span v-else-if="part.metrics.paint_completed > 0" class="badge bg-success">
                        <i class="fas fa-check-circle me-1"></i>All Approved Coated
                      </span>
                      <span v-else class="text-muted small">No QC-Approved Stock</span>
                    </td>
                    <td>
                      <span class="badge bg-purple text-white" v-if="part.metrics.paint_completed > 0">
                        {{ part.metrics.paint_completed }} pcs Coated
                      </span>
                      <span v-else class="text-muted small">—</span>
                    </td>
                    <td>
                      <div v-if="authStore.userRole === 'ADMIN'">
                        <button v-if="part.metrics.paint_ready > 0" class="btn btn-sm btn-purple text-white fw-bold text-nowrap w-100" @click="openPaintModalFromPart(part)">
                          <i class="fas fa-fill-drip me-1"></i> Complete Paint
                        </button>
                        <span v-else class="text-muted small">No Queue</span>
                      </div>
                      <div v-else class="text-center text-muted extra-small">
                        <span v-if="part.metrics.paint_ready > 0" class="badge bg-warning-subtle text-dark border">In Queue</span>
                        <span v-else-if="part.metrics.paint_completed > 0" class="badge bg-purple-subtle text-purple border">Coated</span>
          ...
          <div v-else-if="!projectId">
            <div class="d-flex justify-content-between align-items-center mb-3">
              <h6 class="fw-bold mb-0 text-secondary text-uppercase ls-1"><i class="fas fa-project-diagram me-2 text-primary"></i>Projects Paint Status ({{ filteredProjectsByStatus.length }})</h6>
              <!-- Active vs Completed Toggle (Part 10) -->
              <div class="btn-group btn-group-sm">
                <button class="btn" :class="projectStatusFilter === 'all' ? 'btn-primary' : 'btn-outline-secondary'" @click="projectStatusFilter = 'all'">
                  All ({{ projects.length }})
                </button>
                <button class="btn" :class="projectStatusFilter === 'active' ? 'btn-warning text-dark fw-bold' : 'btn-outline-secondary'" @click="projectStatusFilter = 'active'">
                  Active ({{ activeProjectsCount }})
                </button>
                <button class="btn" :class="projectStatusFilter === 'completed' ? 'btn-success fw-bold' : 'btn-outline-secondary'" @click="projectStatusFilter = 'completed'">
                  Completed ({{ completedProjectsCount }})
                </button>
              </div>
            </div>
            <div class="row g-2">
              <div v-for="proj in filteredProjectsByStatus" :key="proj.id" class="col-md-6 col-lg-4">
                  ...
                  <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                      <h6 class="fw-bold mb-0 text-dark text-truncate me-2" style="max-width: 70%;" :title="proj.name">
                        <i class="fas fa-folder text-purple me-1.5"></i>{{ proj.name || proj.project_code || 'Project' }}
                      </h6>
                      <span class="badge" :class="proj.is_complete ? 'bg-success' : 'bg-purple text-white'" style="font-size: 0.72rem;">
                    ...
                    <div class="d-flex justify-content-between text-muted extra-small border-top pt-1.5">
                      <span>Approved: <strong>{{ proj.qc_approved }}</strong></span>
                      <span>Coated: <strong class="text-purple">{{ proj.paint_completed }}</strong></span>
                    </div>
                  </div>
                  <div class="card-footer bg-light border-0 text-end py-1.5 px-3">
                    <span class="extra-small text-primary fw-bold">Open Paint Tree <i class="fas fa-arrow-right ms-1"></i></span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-else>
            ...
            <div v-if="!selectedJig">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <h6 class="fw-bold mb-0 text-dark">
                  <i class="fas fa-cubes me-1.5 text-purple"></i>JIG Assemblies for {{ activeProject?.name }} ({{ jigs.length }} JIGs)
                </h6>
              </div>
                    ...
                    <div class="card-body p-3">
                      <div class="d-flex justify-content-between align-items-start mb-1">
                        <h6 class="fw-bold mb-0 text-dark text-truncate me-2">
                          <i class="fas fa-layer-group text-purple me-1.5"></i>{{ jig.jig_name }}
                        </h6>
                        <span class="badge" :class="jig.is_complete ? 'bg-success' : 'bg-purple text-white'" style="font-size: 0.72rem;">
                          {{ jig.completion_pct }}% Coated
                        </span>
                      </div>
                      ...
                      <div class="progress mb-2" style="height: 6px;">
                        <div class="progress-bar bg-purple" :style="{ width: jig.completion_pct + '%' }"></div>
                      </div>

                      <div class="d-flex justify-content-between text-muted extra-small border-top pt-1.5">
                        <span>Approved: <strong>{{ jig.metrics?.qc_approved || 0 }}</strong></span>
                        <span>Ready: <strong class="text-warning text-dark">{{ jig.metrics?.paint_ready || 0 }}</strong></span>
                        <span>Coated: <strong class="text-purple">{{ jig.metrics?.paint_completed || 0 }}</strong></span>
                      </div>
                    </div>
                    <div class="card-footer bg-light border-0 text-end py-1.5 px-3">
            ...
            <div v-else-if="selectedJig && !selectedUnit">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <h6 class="fw-bold mb-0 text-dark">
                  <i class="fas fa-th-large me-1.5 text-purple"></i>Units in JIG {{ selectedJig.jig_name }} ({{ selectedJig.units?.length || 0 }} Units)
                </h6>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center mb-2 gap-2">
                  <div>
                    <h6 class="fw-bold mb-0 text-dark d-flex align-items-center flex-wrap gap-2">
                      <span><i class="fas fa-list me-1.5 text-purple"></i>JIG {{ selectedJig.jig_name }} — {{ selectedUnit.unit_no }}</span>
                ...
                <div class="card border shadow-xs">
                  <div class="card-header py-2 px-3 border-bottom d-flex justify-content-between align-items-center"
                    ...
                    <div class="d-flex align-items-center gap-2">
                      ...
                      <h6 class="fw-bold mb-0 text-dark">{{ selectedUnit.unit_no }} — {{ selectedUnitSide === 'COMMON' ? 'Common' : (selectedUnitSide === 'LH' ? 'Left Hand' : 'Right Hand') }} Paint Desk</h6>
                    </div>
                      ...
                      <table class="table table-hover align-middle border-top mb-0 small">
                        <thead class="table-dark sticky-top" style="z-index: 1;">
                          <tr>
                            ...
                            <th>Supplier</th>
                            <th>Paint Status</th>
                            <th style="width: 140px;">{{ authStore.userRole === 'ADMIN' ? 'Paint Action' : 'Status' }}</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr v-for="part in activeUnitSideParts" :key="selectedUnitSide + '_' + part.id">
                            ...
                            <td>
                              <div v-if="part.metrics.paint_ready > 0" class="badge bg-warning text-dark d-block mb-1">
                                <i class="fas fa-paint-roller me-1"></i>{{ part.metrics.paint_ready }} Ready
                              </div>
                              <div v-if="part.metrics.paint_completed > 0" class="badge bg-purple text-white d-block">
                                <i class="fas fa-check-circle me-1"></i>{{ part.metrics.paint_completed }} Coated
                              </div>
                            </td>
                            <td>
                              <div v-if="authStore.userRole === 'ADMIN'" class="d-flex flex-column gap-1">
                                <button v-if="part.metrics.paint_ready > 0" class="btn btn-xs btn-purple text-white fw-bold text-nowrap w-100" @click="openPaintModalFromPart(part)">
                                  <i class="fas fa-fill-drip me-1"></i> Paint
                                </button>
                              ...
                              <div v-else class="text-center">
                                <span v-if="part.metrics.paint_ready > 0" class="badge bg-warning-subtle text-dark border">
                                  <i class="fas fa-paint-roller me-1"></i>Awaiting Paint
                                </span>
                                <span v-else-if="part.metrics.paint_completed > 0" class="badge bg-purple-subtle text-purple border">
                                  <i class="fas fa-check-circle me-1"></i>Coated
                                </span>
                          ...
                          <tr v-if="!(selectedUnitSide === 'LH' ? selectedUnitLhParts : selectedUnitRhParts).length">
                            <td colspan="4" class="text-center py-4 text-muted">No {{ selectedUnitSide }} paint parts in this unit.</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content shadow-lg border-0">
          <div class="modal-header bg-purple text-white">
            <h5 class="modal-title fw-bold"><i class="fas fa-paint-roller me-2"></i>Log Paint Process Completion</h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body" v-if="activePaintTarget">
            <p>Recording surface coating for <strong>{{ activePaintTarget.standard_part_no }}</strong> ({{ activePaintTarget.quantity }} units, {{ activePaintTarget.side }}).</p>
            <div class="mb-3">
            ...
            <div class="mb-3">
              <label class="form-label fw-semibold">Process Notes / Batch ID</label>
              <textarea v-model="paintForm.remarks" class="form-control" rows="2" placeholder="Optional batch notes..."></textarea>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
            <button type="button" class="btn btn-purple text-white fw-bold" @click="submitPaint" :disabled="submitting">
              <i class="fas fa-save me-1"></i> Save Completion
            </button>
          </div>
        </div>
      </div>
...
const activeProjectsCount = computed(() => {
  return projects.value.filter(p => p.eligible_qty > 0 || !p.is_complete).length;
});

const completedProjectsCount = computed(() => {
  return projects.value.filter(p => p.is_complete).length;
});

const filteredProjectsByStatus = computed(() => {
  if (projectStatusFilter.value === 'active') {
    return projects.value.filter(p => p.eligible_qty > 0 || !p.is_complete);
  }
  if (projectStatusFilter.value === 'completed') {
    return projects.value.filter(p => p.is_complete);
  }
...
const activePaintTarget = ref(null);
const error = ref('');
...
const onSearchInput = () => {
  ...
  searchDebounce = setTimeout(async () => {
    ...
    try {
      const response = await axios.get(`/api/v1/paint/hierarchy?search=${encodeURIComponent(searchQuery.value)}&project_id=${projectId.value || ''}&side=${selectedSide.value || ''}`);
