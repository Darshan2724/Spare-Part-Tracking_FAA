...
* **Universal Export Engine (`app/Services/ExportService.php`):**
  ...
  - **Project Jig Excel Export (`/api/v1/projects/{id}/export-jigs`):**
    ...
    - **Columns (14 Columns, A through N):**
      ...
      - **Col N (`Jig Completion %`):** Individual Jig manufacturing completion ratio.
    - **Jig Completion % Formulation:** Strictly calculated per Jig matching the website Jig card: $\min(100, \text{round}((\text{Jig Assembly Completed} / \text{Jig Total Required}) \times 100, 1)) / 100$, formatted as an Excel percentage (`0.0%`). Varies per Jig based on its own production progress (e.g. `LIMOFD10` displays `5.7%` both on the website card and in Excel, `LIMORD70` displays `6.7%`, `LIMOFD20` displays `0.0%`). Never repeats an overall project completion ratio across different Jigs. Zero required parts safely evaluates to `0.0%` (preventing misleading 100% values).
    ...
    - **Dedicated `TOTAL` Summary Row:** Bottom row of each Jig table computes sums across numeric columns E through M (`Total` through `ECN`) with bold styling, thin borders, and that Jig's overall Jig Completion ratio in Column N formatted as `0.0%`.
