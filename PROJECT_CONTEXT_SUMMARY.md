...
* **Universal Export Engine (`app/Services/ExportService.php`):**
  ...
  - **Project Jig Excel Export (`/api/v1/projects/{id}/export-jigs`):**
    ...
    - **Columns (14 Columns, A through N):**
      ...
      - **Col N (`Jig Completion %`):** Individual Jig manufacturing completion ratio populated **strictly in the `TOTAL` summary row only** (fixture/side rows such as LH and RH remain cleanly blank, eliminating side repetition).
    - **Jig Completion % Formulation:** Strictly calculated per Jig matching the website Jig card: $\min(100, \text{round}((\text{Jig Assembly Completed} / \text{Jig Total Required}) \times 100, 1)) / 100$, formatted as an Excel percentage (`0.0%`). Rendered exclusively in the Jig `TOTAL` row (e.g. `LIMOFD10` displays `5.7%` both on the website card and in Excel TOTAL row, `LIMORD70` displays `6.7%`, `LIMOFD20` displays `0.0%`). Zero required parts safely evaluates to `0.0%` (preventing misleading 100% values).
    ...
    - **Dedicated `TOTAL` Summary Row:** Bottom row of each Jig table computes sums across numeric columns E through M (`Total` through `ECN`) with bold styling, thin borders, and that Jig's overall combined Jig Completion ratio in Column N formatted as `0.0%`. Fixture rows (LH/RH) leave Column N blank.
...
1. **Strict Filename-Based BOM Type Determination:**
   - The BOM intake type (`MFG`, `BOP`, `STD`) is authoritatively determined strictly from the **uploaded filename token** using regex delimiter boundary lookaround: `/(?<=^|[^a-zA-Z0-9])(MFG|BOP|STD)(?=[^a-zA-Z0-9]|$)/i`.
   - Delimiters allowed around tokens include `_`, `-`, space, `(`, `)`, `[`, `]`, `.`, `,`, or string start/end.
   - Lookaround boundaries prevent false positive matches on substrings (e.g. `BOPP_Tape`, `Standard_Parts`, `suboptimal`, `manufacturing`).
   - If an uploaded filename contains **no valid token** or contains **multiple conflicting tokens**, preview and import are immediately rejected before any database mutation with a clear error:
     - Missing: `"Invalid BOM filename: Filename must contain MFG, BOP, or STD to identify the BOM intake type (e.g. 'Project_BOP.xlsx')."`
     - Ambiguous: `"Ambiguous BOM type: Filename contains multiple BOM intake tokens (X, Y). The filename must specify exactly one BOM type."`
2. **Workbook-Authoritative Project Identity Extraction:**
   - The Project Code / Project Name is read strictly from **inside the uploaded Excel workbook cells** (under column headers `Project Code`, `Project Name`, `Project`, `Proj Code`, `Project No`), **never inferred or guessed from the filename**.
   - `ProjectIdentityResolver.php` matches candidate codes derived strictly from workbook cells against existing database projects (`project_code`, `name`, or canonical `FA-xxx` regex). Filename text cannot redirect or override project identity.
3. **Repeat Parts Skipping (Never Summed):**
   - When multiple rows with identical composite keys `(project, jig, unit, part_no, side)` exist in an uploaded file (common in BOP and STD sheets), the importer retains the **first occurrence only**, **never sums quantities**, and emits an upload warning banner noting that repeat parts are skipped by default.
4. **Incremental Revision Intake for Remaining Parts:**
   - Every uploaded filename must be unique per batch (identical filenames rejected by `checkDuplicateFilename`).
   - When remaining parts for an existing project need to be uploaded, users append a revision marker at the end of the filename (e.g. `FA-273_BOP_rev1.xlsx`, `FA-273_BOP(1).xlsx`).
   - `reconcileImportRows` matches the existing project from the sheet's project code, filters existing items by `where('part_type', $bomType)`, skips unchanged rows, updates revised quantities, and imports newly added parts with zero duplicate projects or duplicate items.
5. **Quantity Downward Revision Protection:**
   - If an incoming revision decreases required quantity below what has already been received on the shop floor, the row is marked as a `CONFLICT` and blocks application until administrative confirmation.
...
| **2026-09-08** | STD Quick Movement Enterprise Workflow Toolbar & Restrained Semantic Palette Redesign | `StdIntake.vue`, `PROJECT_CONTEXT_SUMMARY.md` | None (Frontend UI Component, Deterministic Grid Layout & Color System) | Redesigns STD Quick Movement action area into a professional enterprise toolbar (`.std-toolbar`) with bounded slate framing, eliminating staggered 4-row pyramid wrapping; provides clean single-row alignment for $\le 3$ active actions and balanced 2-row grid for $> 3$ actions (Row 1: `Receive`, `QC`, `QC Inspection`; Row 2: `Rework`, `ASM`, `Complete`); replaces oversaturated solid blocks with soft-tinted semantic chips (`.std-btn-action`), crisp borders, and dark high-contrast text; preserves 100% of underlying transitions, FIFO ledger math, and 3-way QC routing | Passing (235 tests, 2701 assertions) |
| **2026-09-08** | STD Quick Movement Layout Optimization & Action Label Standardization (`QC`, `QC Inspection`, `ASM`) | `StdIntake.vue`, `PROJECT_CONTEXT_SUMMARY.md` | None (Frontend UI Component, CSS Design System & Label Polish) | Resolves oversized 6-button vertical tower in STD Aggregated Inventory table by replacing Bootstrap `.btn-group` with `.std-quick-actions` inline flex-wrap container and `.std-quick-btn` (`flex: 0 0 auto`, `font-size: 0.72rem`, `padding: 2px 7px`), achieving ~75% reduction in row height (~48px max across 2 neat rows vs. 210px+ previously); standardizes action labels (`To QC` -> `QC`, `QC Route` -> `QC Inspection`, `To Asm` -> `ASM`) and modal triggers while preserving 100% of underlying 3-way QC routing and FIFO ledger transitions | Passing (235 tests, 2701 assertions) |
| **2026-09-08** | STD QC Route 3-Way Atomic Distribution (Rework, Paint, Assembly) & Action Label Rename | `StdIntake.vue`, `StdIntakeService.php`, `StdIntakeController.php`, `routes/api.php`, `StdIntakeAndWorkflowTest.php`, `PROJECT_CONTEXT_SUMMARY.md` | None (Backend Domain Service, API & Frontend Modal Enhancement) | Enhances STD QC Route modal into an intuitive 3-destination distribution dialog with live allocation summary bar, destination-specific steppers, "Max" balance fill helpers, and atomic multi-target routing (`POST /api/v1/std/qc-route`); renames quick-action button from "Fix" / "Fix Done" to "Rework" across table rows and unit breakdown actions; maintains atomic database transactions with row-level locks for split accounting | Passing (235 tests, 2701 assertions) |
...
| **2026-09-10** | FAA-120 Live Diagnostic V2 Optimization: Hierarchy Payload Pruning, ECN Guarding, Dashboard Parallelization & Mobile Cache-First Level 2 UX | `HierarchyService.php`, `DashboardController.php`, `Dashboard.vue`, `mobile/App.js`, `PROJECT_CONTEXT_SUMMARY.md` | None (Algorithmic, serialization pruning, client parallelization, cache-first mobile rendering) | (1) Conditionally guards 4 ECN database queries when project has zero active ECNs; (2) prunes duplicate root collections and unsets heavy Eloquent relations (`project`, `requirements`) reducing JSON payload from ~2.3MB to ~450KB; (3) scopes widget queries (`delayedParts`, `qualityTrend`, `recentEvents`, `supplierQuality`) to selected project; (4) parallelizes `summary` and `hierarchy` in `Dashboard.vue` with stale-while-revalidate caching; (5) implements instant cache-first Level 2 JIG rendering, loading spinner, error retry, and eliminates false "No JIGs match" in `mobile/App.js` | Passing (244 tests, 2820 assertions) |
| **2026-09-10** | Full-System Performance Audit & Optimization (FAA-120 Live Server Diagnostic) | `Dockerfile`, `opcache.ini`, `entrypoint.sh`, `default.conf`, `docker-compose.yml`, `DashboardController.php`, `QuantityCalculationService.php`, `HierarchyService.php`, `Dashboard.vue`, `mobile/App.js`, `2026_09_10_000001_add_operational_performance_indexes.php`, `PerformanceBenchmarkTest.php`, `PerformanceScaleTest.php` | Added FK & composite aggregation indexes on `rework_records`, `paint_records`, `assembly_records`, `qc_inspections`, `receipt_items` | Enabled PHP OPcache (cuts 5.2s baseline to <50ms); Nginx Gzip compression (shrinks 4.3MB payloads by 88% to ~350KB); FastCGI buffers to eliminate disk buffering; PostgreSQL NVMe SSD settings (`random_page_cost=1.1`, `shared_buffers=1GB`, `effective_io_concurrency=200`); single-pass `calculateBulkProjectsMetricsWithPartTypes` (75% query reduction); targeted column selection and raw collection pruning in `HierarchyService` & `priorityMap` (75-80% raw payload reduction, ~40KB Gzip); WebSocket refetch debounce (1.5s) & mobile request deduplication with 60s polling; added 50-100 project scale load tests; scales seamlessly to 50-100 concurrent projects with zero data alteration | Passing |
